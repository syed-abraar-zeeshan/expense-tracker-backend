...
logger.success(
      `Expense created successfully by user: ${req.user.id}`
    );
...
const getExpenses =  async(req, res) => {
    try {
        const expenses = await Expense.find({userId: req.user.id}).populate('category', 'name icon color').sort({ date: -1 }).lean();
